  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="12" >
        <v-card class="pa-3">
          <v-card-title>
            <h2 class="text-h7">Data Pengajuan Training</h2>
            <v-spacer></v-spacer>
            <v-text-field
              v-model="searchQuery"
...
export default {
  ...
  methods: {
        exportToExcel() {
      // Langkah 1: Siapkan data untuk diekspor
      const data = this.filteredList.map((item, index) => ({
        No: index + 1 + (this.currentPage - 1) * this.itemsPerPage,
        Nama: item.nama,
        Judul_Training: item.judul_training,
        Vendor: item.penyelenggara,
        Tempat: item.tempat,
        Jam: item.jam,
        Harga: item.biaya,
      ...
      // Langkah 2: Buat worksheet dari data
      const worksheet = XLSX.utils.json_to_sheet(data);

      // Langkah 3: Buat workbook dan tambahkan worksheet
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Data Pengajuan");

      // Langkah 4: Ekspor workbook ke file Excel
      XLSX.writeFile(workbook, "Data_Pengajuan_Training.xlsx");
    },
  ...
  getStatusClass(status) {
    ...
    Swal.fire({
      title: 'Anda yakin?',
      text: "Data pengajuan ini akan disetujui!",
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        axios.put(`/api/pengajuan/${item.pengajuan_id}`, {
          ...item,
          status: 1 // Set status to Approved (1)
        })
        .then(response => {
          Swal.fire('Berhasil!', 'Pengajuan telah disetujui.', 'success');
          this.fetchPengajuan(); // Refresh the list
        })
        .catch(error => {
          Swal.fire('Gagal!', 'Tidak dapat menyetujui pengajuan.', 'error');
        });
      }
    });
  },
  ...
  confirmReject(item) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        .catch(error => {
          Swal.fire('Gagal!', 'Tidak dapat menolak pengajuan.', 'error');
        });
      }
    });
  },
